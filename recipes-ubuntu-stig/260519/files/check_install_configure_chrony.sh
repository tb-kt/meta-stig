...
# Configure chrony to use authoritative time sources
echo "Configuring chrony to use authoritative time sources..."
chrony_conf="/etc/chrony/chrony.conf"

if [ -f "$chrony_conf" ]; then
    echo "Adding authoritative time sources to $chrony_conf..."

    # Add or update the time servers
    grep -q "server tick.usno.navy.mil" "$chrony_conf" || echo "server tick.usno.navy.mil iburst maxpoll 16" >> "$chrony_conf"
    grep -q "server tock.usno.navy.mil" "$chrony_conf" || echo "server tock.usno.navy.mil iburst maxpoll 16" >> "$chrony_conf"
    grep -q "server ntp2.usno.navy.mil" "$chrony_conf" || echo "server ntp2.usno.navy.mil iburst maxpoll 16" >> "$chrony_conf"
