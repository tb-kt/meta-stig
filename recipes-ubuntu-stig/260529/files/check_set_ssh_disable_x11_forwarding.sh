...
# Configure SSH to disable X11 forwarding
echo "Configuring SSH to disable X11 forwarding..."

# Ensure X11Forwarding is set to "no"
grep -q "^X11Forwarding" "$SSH_CONFIG_FILE" && sed -i 's/^X11Forwarding.*/X11Forwarding no/' "$SSH_CONFIG_FILE" || echo "X11Forwarding no" >> "$SSH_CONFIG_FILE"
