...
# Configure SSH to use only FIPS-validated key exchange algorithms
echo "Configuring SSH to use only FIPS-validated key exchange algorithms..."

# Ensure KexAlgorithms is set to the FIPS-approved list
grep -q "^KexAlgorithms" "$SSH_CONFIG_FILE" && sed -i 's/^KexAlgorithms.*/KexAlgorithms ecdh-sha2-nistp256,ecdh-sha2-nistp384,ecdh-sha2-nistp521,diffie-hellman-group-exchange-sha256/' "$SSH_CONFIG_FILE" || echo "KexAlgorithms ecdh-sha2-nistp256,ecdh-sha2-nistp384,ecdh-sha2-nistp521,diffie-hellman-group-exchange-sha256" >> "$SSH_CONFIG_FILE"
