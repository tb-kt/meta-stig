...
# Configure SSH to prevent remote hosts from connecting to the proxy display
echo "Configuring SSH to prevent remote hosts from connecting to the proxy display..."

# Ensure X11UseLocalhost is set to "yes"
grep -q "^X11UseLocalhost" "$SSH_CONFIG_FILE" && sed -i 's/^X11UseLocalhost.*/X11UseLocalhost yes/' "$SSH_CONFIG_FILE" || echo "X11UseLocalhost yes" >> "$SSH_CONFIG_FILE"
