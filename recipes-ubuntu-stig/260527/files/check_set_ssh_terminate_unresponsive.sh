...
# Configure SSH to terminate unresponsive sessions
echo "Configuring SSH to terminate unresponsive sessions..."

# Ensure ClientAliveCountMax is set to "1"
grep -q "^ClientAliveCountMax" "$SSH_CONFIG_FILE" && sed -i 's/^ClientAliveCountMax.*/ClientAliveCountMax 1/' "$SSH_CONFIG_FILE" || echo "ClientAliveCountMax 1" >> "$SSH_CONFIG_FILE"
