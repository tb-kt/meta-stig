...
# Description: This script configures Ubuntu 22.04 LTS for PKI-based authentication to use local cache for revocation data.
...
# Update cert_policy to ensure 'crl_auto' or 'crl_offline' is included
echo "Configuring certificate policy in $PAM_PKCS11_CONF"
if grep -q "cert_policy" "$PAM_PKCS11_CONF"; then
    echo "Updating existing cert_policy to include 'crl_auto'..."
    sudo sed -i 's|cert_policy = .*;|cert_policy = ca,signature,ocsp_on,crl_auto;|' "$PAM_PKCS11_CONF"
else
    # Add cert_policy if it doesn't exist
    echo "Adding cert_policy configuration..."
    sudo sed -i '/pkcs11_module opensc {/,/}/ s|}|    cert_policy = ca,signature,ocsp_on,crl_auto;\n}|' "$PAM_PKCS11_CONF"
fi

echo "PKI-based authentication revocation cache configuration is complete."
