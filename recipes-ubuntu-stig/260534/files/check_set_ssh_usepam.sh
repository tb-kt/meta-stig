...
# Configure SSH to use PAM for strong authentication
echo "Configuring SSH to use PAM for strong authentication..."

# Ensure UsePAM is set to "yes"
grep -q "^UsePAM" "$SSH_CONFIG_FILE" && sed -i 's/^UsePAM.*/UsePAM yes/' "$SSH_CONFIG_FILE" || echo "UsePAM yes" >> "$SSH_CONFIG_FILE"
