...
# Configure SSH to terminate unresponsive sessions after 10 minutes
echo "Configuring SSH to terminate unresponsive sessions after 10 minutes..."

# Ensure ClientAliveInterval is set to "600" (10 minutes)
grep -q "^ClientAliveInterval" "$SSH_CONFIG_FILE" && sed -i 's/^ClientAliveInterval.*/ClientAliveInterval 600/' "$SSH_CONFIG_FILE" || echo "ClientAliveInterval 600" >> "$SSH_CONFIG_FILE"
